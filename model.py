...
class Transofrmer(torch.nn.Module):
    def __init__(self, BatchSize, ROIs, heads, BOLDs, q):
        ...
        self.threshold = Variable(self.threshold.float(),requires_grad=True).to("cuda")#这个是叶子张量吗？如果是的话就不应该设置grad=true
    ...
    def forward(self, x):
        ...
        relation3 = F.softmax(relation1, dim=3)
    ...
    def forward(self, relation_SE):
        ...
        for i in range(relation_SE.shape[0]):
            for j in range(relation_SE.shape[1]):
                relation_SE[i, j] =relation_SE[i, j] * output[0, 0, i, j].long()
        return relation_SE
...
class MLP(torch.nn.Module):
    ...
    def forward(self, fusion_output):
        ...
        fusion_output2=fusion_output1.reshape(fusion_output.shape[0],fusion_output.shape[1])
...
class whole_network(torch.nn.Module):
    ...
    def forward(self, BOLD_signals):  # BOLD_signals[batch_size,ROIs,features]
        ...
        relation_output2 = self.se(relation_output)  # relation[batch_size,heads,ROIs,ROIs]
