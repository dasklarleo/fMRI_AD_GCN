...
class whole_network(torch.nn.modules):
    ...
    feature_nums,dilated_parameter_k):
        ...
        self.transofrmer=Transofrmer(batch_size,ROI_nums,heads,BOLD_nums,q)
    ...
    def forwara(self,BOLD_signals):        #BOLD_signals[batch_size,ROIs,features]

        relation=self.transofrmer(BOLD_signals)        #relation[batch_size,heads,ROIs,ROIs]

        relation=self.se(relation)        #relation[batch_size,heads,ROIs,ROIs]

        BOLD_signals=BOLD_signals.view(self.batch_size,1,self.ROI_nums,self.BOLD_nums)        #BOLD_signals[batch_size,1,ROI_nums,BOLD_nums]
        ...
        GCN_output=self.gcn_layer8(GCN_output) #GCN_output:[batch_size,heads,ROIs,features]

        fusion_output=self.fusion(GCN_output) #fusion_output:[batch_size,1,ROIs,features]
        fusion_output=fusion_output.view(self.batch_size,self.ROI_nums,self.BOLD_nums)

        predict_labels=self.mlp(fusion_output)# predict_labels:[batch_size,labels]
