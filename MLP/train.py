...
if __name__ == '__main__':
    ...
    loss_average=0
    # train the network
    for epoch in range(80):# one epoch means that the net has seen all the data in the dataset
        for batch_idx,(data,target) in enumerate(train_loader):
            ...
            # forward
            scores=net(data)
            loss=criterion(scores,target)-0.001*net.threshold
            #print(loss)
        ...
        loss_average=0
        print("*"*10)
        threshold.append(net.threshold)
        check_accuracy(test_loader,net)
        print(net.threshold.item())
